...
public class MentionOutLinkExtractor {
  ...
  public static class TweetReducer
	  ...
	  private HashSet<String> _tmpUniqueLink = new HashSet<String>();
      ...
      public void reduce(Text key, Iterable<Text> values,  
    	  ...
    	  _tmpUniqueLink.clear();
    	  _buffer.delete(0, _buffer.length());
          for (Text val : values) {
        	  String s = val.toString();
        	  if (!_userDict.contains(s) || s.equals(key.toString())) {
    			  continue;
    		  } 
        	  if (_tmpUniqueLink.add(s)) {

        	  }
          }          
          
          _buffer.append(new Long(_tmpUniqueLink.size()).toString() + "\t");
          for (String s : _tmpUniqueLink) {
    		  _buffer.append(s);
    	  	  _buffer.append("\t");
          }          
    	  ...
    	  if (_buffer.length() > 0) {
    		  newKey.set(key);
    		  newVal.set(_buffer.substring(0, _buffer.length()-1));	// remove the last '\t'
    	  	  context.write(newKey, newVal);
      	  }
      } 
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    job.setJarByClass(MentionOutLinkExtractor.class);//主类
