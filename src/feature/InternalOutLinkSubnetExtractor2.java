...
public class InternalOutLinkSubnetExtractor2 {
  ...
  public static class TweetReducer
    ...
    public void reduce(StrLongPair key, Iterable<Text> values,  
    	...
    	for (Text val : values) {
    		count ++;
    		/*
    		 * Note: in iterator, both key and value are changing! 
    		 */
    		System.out.println(key.getFirst()+ "|" + key.getSecond() + "|" + count);
    		newKey.set(key.getFirst()+ "|" + key.getSecond() + "|" + count);
  ...
  extends Reducer<Text, Text, Text, Text> {
	  ...
	  public void reduce(Text key, Iterable<Text> values,  
		  ...
		  newVal.set(""
		  		  + "\t" + new Long(_subnetOutLinkCountMap.size()).toString() 				
  ...
  public static boolean MR1(String[] otherArgs, Configuration conf) throws Exception {
	  Job job = new Job(conf, "internal outlink subnet extractor");
	    job.setJarByClass(InternalOutLinkSubnetExtractor2.class);//主类
