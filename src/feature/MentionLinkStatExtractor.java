...
public class MentionLinkStatExtractor {
      ...
      public void map(Object key, Text value, Context context)  
          ...
          if (pos != -1) {          	 
              ...
              if (pos2 != -1) {
            	  ...
            	  ArrayList<String> toUserList = _parser.getMentionedUserList(rest);
  ...
  public static class TweetReducer
	  ...
	  private final Text newVal = new Text();
	  private Configuration _conf = null;
	  private HashSet<String> _tmpOutLinkSet = new HashSet<String>();
	  private HashSet<String> _tmpInLinkSet = new HashSet<String>();

	  private HashSet<String> _userDict = new HashSet<String>();
	  //private TreeSet<String> _userDict = new TreeSet<String>();
		...
		protected void setup(Context context
			...
			while ((line = br.readLine()) != null) {
				if (!line.isEmpty()) {
					int pos = line.indexOf("\t");
					if (pos != -1) {
						_userDict.add(line.substring(0, pos));
					}
					else {
						_userDict.add(line);
					}
      ...
      public void reduce(Text key, Iterable<Text> values,  
    	  ...
    	  if (!_userDict.contains(key.toString()))
    		  return;
    	  ...
    	  long uniqueTotalDLink = 0;              	  
    	  
          _tmpOutLinkSet.clear();
          _tmpInLinkSet.clear();
          ...
          for (Text val : values) {
        	  ...
        	  if (!(_userDict.contains(from) && _userDict.contains(to))) {
        		  continue;
        	  }       		
        	  if (from.equals(to)) {	//ignore self link
        		  ...
        		  totalOut ++;
        		  if (_tmpOutLinkSet.add(s)) {
        			  uniqueTotalOut ++;
        		  }
        	  }
        	  else if (to.equals(key.toString())) {
        		  totalIn ++;
        		  if (_tmpInLinkSet.add(s)) {
        			  uniqueTotalIn ++;
        		  }
        	  }
          }          
          
          for (String s : _tmpOutLinkSet) {	 
        	  int pos = s.indexOf("\t");
        	  from = s.substring(0, pos);
        	  to = s.substring(pos + 1);
        	  String reversed = to + "\t" + from;
        	  if (_tmpInLinkSet.contains(reversed)) {	// find a pair of bi-directional links
        		  uniqueTotalDLink ++;
        	  }        	    	  
          }    
          ...
          double ratio1 = 0.0;
          if (uniqueTotalOut > 0)
        	  ratio1 = uniqueTotalDLink / (double)uniqueTotalOut;
          double ratio2 = 0.0;
          if (uniqueTotalIn > 0)
              ratio2 = uniqueTotalDLink / (double)uniqueTotalIn;
    	  
    	  newKey.set(key);
    	  newVal.set(new Long(uniqueTotalDLink).toString() 
    			  + "\t" + new Long(uniqueTotalOut).toString() 
    			  + "\t" + new Double(ratio1).toString()
    			  + "\t" + new Long(uniqueTotalIn).toString()
    			  + "\t" + new Double(ratio2).toString()
    			  + "\t" + new Long(totalOut).toString() 
    			  + "\t" + new Long(totalIn).toString()
    			  	);
    	  context.write(newKey, newVal);
      }     
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    job.setJarByClass(MentionLinkStatExtractor.class);//主类
