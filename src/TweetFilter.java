...
public class TweetFilter {
	...
	private static final String TWEET_DICT_FILE = "tweet.dict.file";
  ...
  public static class TweetMapper
	  ...
	  private HashSet<String> _tweetDict = new HashSet<String>();
      ...
      String parseContent(String body) {
    	  ...
    	  while (tokenizer.hasMoreTokens()) {
    		  ...
    		  if (word.charAt(0) == '#') {
	    		  ...
	    		  if (!_tweetDict.contains(newWord)) {
	    			  continue;
	    		  }	    		  
      ...
      @Override
		protected void setup(Context context
			...
			String tweetDictFileName = _conf.get(TWEET_DICT_FILE);			
			...
			while ((line = br.readLine()) != null) {
				if (!line.isEmpty()) {
					...
					if (pos == -1)
						...
						_tweetDict.add(line.substring(0, pos));
				}
			}			
			br.close();
		}	        
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (otherArgs.length != 3) {
      System.err.println("Usage: TweetFilter <hdfs-dict-path (with hdfs prefix)> <hdfs-indir> <hdfs-outdir>");
      System.exit(2);
    }
    Job job = new Job(conf, "tweet filter");
    job.setJarByClass(TweetFilter.class);//主类
	...
	jconf.set(TWEET_DICT_FILE, filename);
	URI userProgUri = new URI(userProgOnHDFS + "#_tweetDict");
